...
<body class="bg-slate-50 text-slate-900 flex min-h-screen">
    ...
    <aside class="w-64 bg-indigo-900 text-indigo-100 flex flex-col p-6 space-y-8 sticky top-0 h-screen">
        ...
        <div class="pt-4 border-t border-indigo-800/60">
            <form action="{{ route('admin.logout') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin keluar dari Panel Admin?')">
                @csrf
                <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 text-indigo-300 hover:bg-rose-600 hover:text-white rounded-xl font-bold transition cursor-pointer text-left text-sm">
